...
很多人同时拥有 Mac 和 Windows 两台机器，也可能在两台机器上玩同一款游戏。

如果玩的是 Steam 游戏，那直接用 Steam 自带的存档同步功能就够了，不用折腾。

麻烦的是**本地游戏**——不管是通过 CrossOver 在 Mac 上跑的 Windows 游戏，
还是直接装在硬盘上的游戏，它们的存档散落在各自的文档目录、AppData、游戏安装目录里，
两台机器之间要同步只能手动拷贝，既容易忘也容易拷错版本。

SaveSync 就是为了解决这件事：在 macOS 和 Windows 两个系统下玩本地游戏时，
把存档自动打包上云，换机器时一键下载覆盖，不用再手动折腾。
